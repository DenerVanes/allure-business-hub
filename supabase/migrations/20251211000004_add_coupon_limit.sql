-- Adicionar campos de limite de cupons na tabela promotions
ALTER TABLE public.promotions
ADD COLUMN IF NOT EXISTS limite_cupons_ativo BOOLEAN NOT NULL DEFAULT false,
ADD COLUMN IF NOT EXISTS limite_cupons INTEGER DEFAULT 0 CHECK (limite_cupons >= 0);

-- Atualizar função validate_coupon_only para verificar limite de cupons
...
BEGIN
  ...
  -- VERIFICAR LIMITE DE CUPONS (se ativado)
  IF v_promotion.limite_cupons_ativo AND v_promotion.limite_cupons > 0 THEN
    ...
    v_debug_log := v_debug_log || ' | Cupons utilizados: ' || v_cupons_utilizados::TEXT || ' | Limite: ' || v_promotion.limite_cupons::TEXT;
    
    IF v_cupons_utilizados >= v_promotion.limite_cupons THEN
      -- Desativar promoção automaticamente se atingiu o limite
      ...
      RETURN json_build_object(
        'valid', false,
        'error', 'Promoção expirada ou desativada',
        'debug', 'Limite de cupons atingido. Cupons utilizados: ' || v_cupons_utilizados::TEXT || ' | Limite: ' || v_promotion.limite_cupons::TEXT
      );
    END IF;
  END IF;
...
-- Atualizar função validate_and_use_coupon para verificar limite e desativar automaticamente
