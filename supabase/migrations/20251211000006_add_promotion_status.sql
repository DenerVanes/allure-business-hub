-- ============================================
-- Migração: Adicionar campo status para controle de cupons
-- ============================================
-- 
-- OBJETIVO:
-- 1. Adicionar campo status na tabela promotions para controlar pausado/cancelado
-- 2. Atualizar funções RPC para verificar status antes de validar cupom
-- ============================================

-- Adicionar campo status na tabela promotions
ALTER TABLE public.promotions
ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'ativo' CHECK (status IN ('ativo', 'pausado', 'cancelado'));

-- Atualizar registros existentes para ter status 'ativo' se ativa = true
...
-- Atualizar função validate_coupon_only para verificar status
...
BEGIN
  ...
  -- VERIFICAR STATUS DO CUPOM (pausado ou cancelado)
  IF v_promotion.status = 'cancelado' THEN
    RETURN json_build_object(
      'valid', false,
  ...
  IF v_promotion.status = 'pausado' THEN
    RETURN json_build_object(
      'valid', false,
      'error', 'A promoção finalizou'
    );
...
-- Atualizar função validate_and_use_coupon para verificar status
